...
- 后端对文本能力模型绑定和提示词优化的 `system_prompts` 组合内容做 60 秒内存缓存；后台创建/编辑提示词优化系统提示词后会清理对应缓存，保存「AI 文本能力」设置后会清理文本能力模型绑定缓存。系统提示词不会注入生图、生视频、脚本生成、提示词生成、分镜或工具等其他功能。
...
- 后台「功能开关」明确 AI 文本能力入口；「系统设置 → AI 文本能力」配置模型 ID/积分；「内容管理 → 系统提示词」仅配置提示词优化系统提示词，其他文本功能使用自身的内置指令。
...
视频任务参数链路会保留 `audioMode`、`preserveAudio`、`referenceMode`、可选高级参数 `seed/fps/audioUrl/audioFileId` 和清洗后的 `inputAssets`。档位能力表 `tier_capabilities` 包含 `supported_audio_modes/default_audio_mode`、`max_reference_images`、`max_video_urls` 和 `max_audio_urls`；公开接口返回 `capabilities.audioModes/defaultAudioMode/maxReferenceImages/inputMode/referenceUploadMode/minReferenceImages/requiredReference/inputMediaTypes/maxVideoUrls/maxAudioUrls/advancedParams`。模型配置显式上限或远端媒体参数 `maxItems` 优先于档位旧默认值，公共能力与任务校验共用有效上限。文生图在小程序中始终纯文字输入；图生图继续使用主图/参考图上传，图片编辑始终使用大上传卡片，并按模型上限允许单图或多图。图片预览区默认 4 个槽位，上限小于 4 时超出槽位置灰并禁止操作，上限大于 4 时显示真实数量。`advancedParams` 来自模型显式 `advanced_params/supported_advanced_params` 或 `param_names/input_keys`，只保留小程序可渲染的 `seed/fps/audioUrl`；为空时前端不展示高级参数入口，也不提交草稿里残留的高级参数。`referenceUploadMode=first_frame` 表示单首图图生视频，`reference_images` 表示多参考素材参考生视频，`first_last` 表示首尾帧，`source_video` 表示视频编辑；小程序的「图生视频」和「参考生视频」都提交后端 `image_to_video`，通过 `referenceMode` 区分 UI 和 provider 参数映射。单一能力项也要返回给前端并显示为锁定态，例如固定 `8s`、固定 `1080p`、固定 `audio/silent`。`minReferenceImages/maxReferenceImages/maxVideoUrls/maxAudioUrls` 控制素材池，后端创建任务时会按最少和最多数量二次校验；首尾帧固定首图/尾图 2 张；视频编辑在绑定模型 `maxVideoUrls > 1` 时可提交多个源视频。`inputAssets` 不再只是结果页展示数据：图片素材会进入参考图池，视频素材会进入 `videoUrl/videoUrls`，音频素材会进入 `audioUrl/audioUrls`，外链素材必须是公网 `http/https` URL。
