...
- 后端只缓存文本能力模型绑定和提示词优化的 `system_prompts` 组合内容，TTL 为 60 秒；后台新增或编辑提示词优化系统提示词后会清理对应缓存，保存「AI 文本能力」设置后会清理文本能力模型绑定缓存。
- 提示词优化优先使用「AI 模型管理 → 功能页配置 → 提示词优化」下的功能档位主模型和备用模型；请求可传 `tierKey` 或 `tierId` 在已绑定档位间切换。旧的 `ai.prompt_optimize.model_id` 仅在没有可用档位绑定时兼容兜底。
- 系统提示词配置仅服务于提示词优化：在「内容合规 → 系统提示词」维护 `targetFeature=prompt_optimize` 的启用内容，并和内置深度补全规则合并注入提示词优化模型；生图、生视频、脚本生成、提示词生成、分镜和工具不会读取这张表。
...
- DeepSeek is seeded as `provider_key=deepseek`, `provider_type=openai_compatible`, Base URL `https://api.deepseek.com`; the legacy `ai.prompt_optimize.model_id` value is retained only as a compatibility fallback, while current prompt optimization should bind the model through the `prompt_optimize` feature tier. Configure `DEEPSEEK_API_KEY` or paste the key in admin; SQL seeds do not contain plaintext keys.
- Hongniao uses `provider_key=hongniao`, `provider_type=hongniao`, `X-API-Key`, and the default Base URL `https://open.hongniaoai.com/v1`. Migration `20260707_002_hongniao_open_domain_and_sync_metadata.sql` only upgrades old `https://hongniaoai.com` provider URLs; model rows are refreshed by the strict sync action from `GET /v1/models`. Sync stores common capability fields plus `config.remote_parameters`, updates upstream cost metadata, and preserves existing sale price/display/status. If a synced Hongniao model disappears upstream, preview shows it in `removals`; apply archives the complete model snapshot and hard-deletes the live model after removing tier bindings, fallback rules, capability rows and price rules. Historical tasks/cost logs resolve the archived model snapshot. Video create/query use `POST /v1/videos` and `GET /api/v1/videos/{id}`; image create/query use `POST /v1/images` and `GET /api/v1/images/{id}`. Configure `HONGNIAO_API_KEY`; real generation tests may consume upstream balance.
...
| `model_features` / `model_tiers` | 新增 `tool_prompt_reverse`、`tool_cutout` 两个功能页配置入口 | 工具模型绑定迁移到「微信配置 → 工具页配置」。`AI 模型管理 → 功能页配置` 默认隐藏工具功能；反推提示词调用绑定的 OpenAI-compatible 视觉/多模态模型，支持按 `tierKey/tierId` 切换档位，未绑定或调用失败时明确报错并退款；智能抠图先保留绑定入口，运行时仍使用本地轻量算法 |
...
| `POST /api/v1/admin/real-models/:id/test` | Hongniao AI 供应商 `provider_type=hongniao` 请求使用 `X-API-Key`；默认域名为 `https://open.hongniaoai.com/v1`。后台严格同步实时读取 `/v1/models`，写入常用能力字段、上游成本和 `config.remote_parameters`；远端删除模型会先归档快照，再硬删除 live model 并解除绑定 | 视频创建走 `POST /v1/videos`、查询走 `GET /api/v1/videos/{id}`；图片创建走 `/v1/images`、查询走 `/api/v1/images/{id}`；配置 `HONGNIAO_API_KEY` 或在后台粘贴 Key 后可测试，真实测试可能消耗上游额度 |
...
文生图始终是纯文字输入，不显示参考图入口。图生图使用主图和参考图槽位，图片编辑始终使用大上传卡片；两者的图片数量由绑定模型实际参数与档位能力合并后的 `capabilities.maxReferenceImages` 控制，后端创建任务时会按能力二次校验。图片编辑模型上限大于 1 时可通过“继续上传”和预览槽位追加多张图片。预览区默认显示 4 个槽位，上限小于 4 时超出上限的槽位置灰并禁止操作，上限大于 4 时显示真实数量。视频档位的参考图仍由 `referenceUploadMode` 和 `min/maxReferenceImages` 控制；视频编辑默认支持单源视频，但当绑定模型返回 `maxVideoUrls > 1` 时，小程序会按该上限显示并上传多个源视频，后端也按同一上限校验。
...
- `POST /tools/process`：请求 `{ toolKey, fileIds, params, tierKey?, tierId? }`。图片类工具的 `fileIds` 使用 `/files/upload` 返回的文件 ID，输出为 `{ toolKey, outputs, prompt?, usageSource, pointsCost }`。`prompt_reverse` 按 `tool_prompt_reverse` 功能档位绑定调用模型，可用 `tierKey/tierId` 切换档位；未绑定可用模型或调用失败返回错误并退款，不回退到本地伪提示词；其它工具返回私有图片文件 `outputs[].url`，访问需用户 token。
...
反推提示词绑定 `tool_prompt_reverse` 档位后，会按绑定模型和供应商 Base URL/API Key 调用 OpenAI-compatible `chat/completions`，消息中带 `image_url` data URL。请求可传 `tierKey` 或 `tierId` 切换档位；若绑定模型不可用、供应商不兼容或调用失败，会明确报错并退款，不回退到本地伪提示词。智能抠图已提供 `tool_cutout` 功能页配置入口，用于后续绑定供应商抠图/图片编辑模型；当前运行时仍是本地简单背景移除算法。
...
- `POST /admin/models/providers/:id/sync`：从供应商接口同步可识别模型和能力参数。请求体 `{ "mode": "preview" }` 只拉取远端模型并返回 `{ additions, updates, removals, skipped, failures }` 供后台弹窗确认，不写数据库；`{ "mode": "apply" }` 会重新拉取并应用变动，返回新增/更新/硬删除数量以及 `bindingDeleted/fallbackDeleted`。严格供应商 `xiaoma`、`hongniao`、`agnes_ai` 只有在完整目录获取成功时才允许删除；目录为空或接口失败会阻止 apply。新增模型会自动入库；已有模型只覆盖 `model_type/query_task_url/api_cost_cents/config` 中的上游成本、查询端点、常用能力字段和只读原始参数，售卖价格 `points_cost`、启停状态、显示名、档位绑定、兜底规则和供应商 API Key 不会被覆盖。远端删除的模型先写入 `ai_model_catalog_archive`，再删除 live model，并清理前台档位绑定、兜底规则、能力和价格规则；历史任务/成本日志通过归档快照显示原模型。Agnes 的能力字段来自官方模型文档；无法确定的模型只写入保守能力并标记 `capability_confidence=inferred`，不得据此自动扩展档位能力。
