...
router.get('/providers', adminAuthMiddleware, async (_req: Request, res: Response) => {
  try {
    const rows = await query<any>('SELECT id, name, provider_key, provider_type, api_base_url, api_key, default_timeout, default_retry, status, last_health_check, health_status, remark, created_at FROM ai_model_providers WHERE deleted_at IS NULL ORDER BY id');
  ...
  } catch { error(res, ErrorCodes.SERVER_ERROR, 'Failed to get providers'); }
});
...
router.post('/providers', adminAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    if (!name || !resolvedKey || !apiBaseUrl || !apiKey) { error(res, ErrorCodes.PARAM_ERROR, 'Missing fields'); return; }
  ...
  } catch { error(res, ErrorCodes.SERVER_ERROR, 'Failed to create provider'); }
});
...
router.put('/providers/:id(\\d+)', adminAuthMiddleware, async (req: Request, res: Response) => {
  ...
  } catch { error(res, ErrorCodes.SERVER_ERROR, 'Failed to update provider'); }
});

router.delete('/providers/:id(\\d+)', adminAuthMiddleware, async (req: Request, res: Response) => {
  try {
    const result = await query<any>('UPDATE ai_model_providers SET deleted_at = NOW(3) WHERE id = ? AND deleted_at IS NULL', [parseInt(req.params.id)]);
    if ((result as any).affectedRows === 0) { error(res, ErrorCodes.NOT_FOUND, '供应商不存在', 404); return; }
    success(res, { deleted: true });
  } catch { error(res, ErrorCodes.SERVER_ERROR, '删除供应商失败'); }
});
...
router.post('/providers/:id(\\d+)/sync', adminAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    for (const m of newModels) {
      ...
      // Guess model_type from name and tags
...
router.get('/', adminAuthMiddleware, async (req: Request, res: Response) => {
  try {
    const rows = await query<any>('SELECT m.*, p.name as provider_name, p.provider_type FROM ai_models m JOIN ai_model_providers p ON p.id = m.provider_id ORDER BY m.sort_order DESC, m.id');
    success(res, rows.map((r: any) => ({
  ...
  } catch { error(res, ErrorCodes.SERVER_ERROR, 'Failed to get models'); }
});
...
router.put('/:id(\\d+)', adminAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    if (sets.length === 0) { error(res, ErrorCodes.PARAM_ERROR, 'No fields'); return; }
    vals.push(id);
    await query('UPDATE ai_models SET ' + sets.join(', ') + ' WHERE id = ?', vals);
    success(res, { updated: true });
  } catch { error(res, ErrorCodes.SERVER_ERROR, 'Failed'); }
});

router.delete('/:id(\\d+)', adminAuthMiddleware, async (req: Request, res: Response) => {
  try {
    const result = await query<any>('UPDATE ai_models SET deleted_at = NOW(3) WHERE id = ? AND deleted_at IS NULL', [parseInt(req.params.id)]);
    if ((result as any).affectedRows === 0) { error(res, ErrorCodes.NOT_FOUND, '模型不存在', 404); return; }
    success(res, { deleted: true });
  } catch { error(res, ErrorCodes.SERVER_ERROR, '删除模型失败'); }
});
...
router.post('/:id(\\d+)/capabilities', adminAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    if (!Array.isArray(capabilities)) { error(res, ErrorCodes.PARAM_ERROR, 'Invalid'); return; }
