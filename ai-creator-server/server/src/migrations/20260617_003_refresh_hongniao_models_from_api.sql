-- Refresh Hongniao models with a new migration key so deployed databases that
-- already ran older Hongniao migrations still receive the current model set.
-- Source: GET https://hongniaoai.com/v1/models checked on 2026-06-17.
-- Current response: 1 image_generation model and 11 video_generation models.
-- This migration never stores a plaintext API key.
...
DROP TEMPORARY TABLE IF EXISTS tmp_hongniao_refresh_video_models;
CREATE TEMPORARY TABLE tmp_hongniao_refresh_video_models (
...
INSERT INTO tmp_hongniao_refresh_video_models
...
SELECT @hongniao_id, 'image_to_video', 'hongniao_grok_video_10', 'Hongniao Grok 1.0 Reference Video', 'Hongniao Grok Video 1.0',
...
UNION ALL
SELECT @hongniao_id, 'image_to_video', 'hongniao_grok_video_15', 'Hongniao Grok 1.5 First Frame Video', 'Hongniao Grok Video 1.5',
 ...
 WHERE @hongniao_id IS NOT NULL
UNION ALL
SELECT @hongniao_id, 'image_to_video', 'hongniao_sdquan2_reference', 'Hongniao SDQuan 2 Reference Video', 'Hongniao SDQuan 2',
 ...
 WHERE @hongniao_id IS NOT NULL
UNION ALL
SELECT @hongniao_id, 'image_to_video', 'hongniao_xb_sora2_first_frame', 'Hongniao Sora2 First Frame Video', 'Hongniao XB Sora2',
 ...
 WHERE @hongniao_id IS NOT NULL
UNION ALL
SELECT @hongniao_id, 'video_create', 'hongniao_sora2z_video', 'Hongniao Sora 2-Z', 'Hongniao Sora 2-Z',
 ...
 WHERE @hongniao_id IS NOT NULL
UNION ALL
SELECT @hongniao_id, 'image_to_video', 'hongniao_veo_omni_flash', 'Hongniao Veo Omni Flash', 'Hongniao Veo Omni Flash',
 ...
 WHERE @hongniao_id IS NOT NULL
UNION ALL
SELECT @hongniao_id, 'first_last_frame_video', 'hongniao_p_weo31_first_last', 'Hongniao P-weo3.1 First Last Frame', 'Hongniao P-weo3.1',
 ...
 WHERE @hongniao_id IS NOT NULL
UNION ALL
SELECT @hongniao_id, 'first_last_frame_video', 'hongniao_veo31_xs_first_last', 'Hongniao Veo 3.1 XS First Last Frame', 'Hongniao Veo 3.1 XS',
 ...
 WHERE @hongniao_id IS NOT NULL
UNION ALL
SELECT @hongniao_id, 'image_to_video', 'hongniao_kuaile_reference', 'Hongniao Kuaile 1.0 Reference Video', 'Hongniao Kuaile 1.0',
 ...
 WHERE @hongniao_id IS NOT NULL
UNION ALL
SELECT @hongniao_id, 'image_to_video', 'hongniao_quanneng_j_reference', 'Hongniao Quanneng J Reference Video', 'Hongniao Quanneng J',
 ...
 WHERE @hongniao_id IS NOT NULL
UNION ALL
SELECT @hongniao_id, 'video_edit', 'hongniao_quanneng20_video_edit', 'Hongniao Quanneng 2.0 Video Reference', 'Hongniao Quanneng 2.0',
...
UPDATE ai_models m
JOIN tmp_hongniao_refresh_video_models x
...
SELECT x.provider_id, CONCAT('Hongniao-', x.api_model_name), x.display_name, 'video', x.sub_type,
       ...
       'active', NOW(3), NOW(3)
  FROM tmp_hongniao_refresh_video_models x
 WHERE NOT EXISTS (
...
SELECT f.id, x.tier_name, x.tier_key,
       CONCAT(x.display_name, ' powered by Hongniao. Parameters follow current /v1/models metadata.'),
...
DELETE b
  ...
  JOIN model_tiers t ON t.id = b.tier_id
  JOIN tmp_hongniao_refresh_video_models x ON x.tier_key COLLATE utf8mb4_unicode_ci = t.tier_key COLLATE utf8mb4_unicode_ci
  LEFT JOIN ai_models m ON m.id = b.model_id
                       AND m.provider_id = x.provider_id
                       AND m.api_model_name COLLATE utf8mb4_unicode_ci = x.api_model_name COLLATE utf8mb4_unicode_ci
                       AND m.deleted_at IS NULL
 WHERE b.binding_type = 'primary'
   AND x.provider_id IS NOT NULL
   AND m.id IS NULL;
...
DROP TEMPORARY TABLE IF EXISTS tmp_hongniao_refresh_image_models;
CREATE TEMPORARY TABLE tmp_hongniao_refresh_image_models (
  provider_id BIGINT UNSIGNED NOT NULL,
...
INSERT INTO tmp_hongniao_refresh_image_models
...
SELECT @hongniao_id, 'HongniaoGptImage2', 'Hongniao GPT Image 2', 'gpt-image-2', 'gpt-image-2', 0, 9690,
       JSON_OBJECT(
         ...
         'description', 'Current Hongniao image model from /v1/models. The upstream response does not expose price or params, so this uses the existing GPT Image 2 mapping.'
       ),
       'Current Hongniao image model from /v1/models. Uses POST /v1/images and GET /api/v1/images/{id}.'
 WHERE @hongniao_id IS NOT NULL;

UPDATE ai_models m
JOIN tmp_hongniao_refresh_image_models x
  ON x.provider_id = m.provider_id
 AND x.api_model_name COLLATE utf8mb4_unicode_ci = m.api_model_name COLLATE utf8mb4_unicode_ci
   SET m.name = x.name,
       m.display_name = x.display_name,
       m.model_type = 'image',
       m.sub_type = 'text2img',
       m.upstream_model_code = x.upstream_model_code,
       m.is_async = 1,
...
INSERT INTO ai_models
  (provider_id, name, display_name, model_type, sub_type, api_model_name, upstream_model_code, is_async,
   ...
   timeout_seconds, retry_times, retry_delay_ms, daily_limit, daily_limit_per_user,
   max_concurrency, priority, points_cost, api_cost_cents, sort_order, config, remark, status, created_at, updated_at)
SELECT x.provider_id, x.name, x.display_name, 'image', 'text2img',
       x.api_model_name, x.upstream_model_code, 1,
       '/api/v1/images/{id}', JSON_OBJECT(), '',
       JSON_OBJECT('queued','queued','processing','processing','completed','completed','failed','failed'), JSON_OBJECT(),
       600, 2, 5000, 0, 0,
       2, 0, x.points_cost, 0, x.sort_order, x.config_json, x.remark, 'active', NOW(3), NOW(3)
  FROM tmp_hongniao_refresh_image_models x
 WHERE NOT EXISTS (
   SELECT 1 FROM ai_models m
    WHERE m.provider_id = x.provider_id
      AND m.api_model_name COLLATE utf8mb4_unicode_ci = x.api_model_name COLLATE utf8mb4_unicode_ci
...
-- Soft-delete obsolete HongNiao image models no longer in the /v1/models API response as of 2026-06-18.
-- Only gpt-image-2 remains; Nano Banana variants have been removed upstream.
UPDATE ai_models
   SET deleted_at = NOW(3), updated_at = NOW(3), status = 'inactive'
 WHERE provider_id = @hongniao_id
   AND api_model_name IN ('gemini-3-pro-image-preview', 'gemini-3.1-flash-image-preview')
   AND deleted_at IS NULL;

-- Clean up tier bindings for deleted models
DELETE b
  FROM tier_model_bindings b
  JOIN ai_models m ON m.id = b.model_id
 WHERE m.provider_id = @hongniao_id
   AND m.api_model_name IN ('gemini-3-pro-image-preview', 'gemini-3.1-flash-image-preview')
